...
class XmlGenerator
{
    ...
    public function streamCombinedXmlFile($activities)
    {
        // Send the XML header
        echo '<?xml version="1.0" encoding="UTF-8"?>' . "\n";
        echo '<iati-activities version="2.03" generated-datetime="' . gmdate('c') . '">' . "\n";
        echo '<!-- Generated By IATI Publisher -->' . "\n";

        // For each activity, generate its XML and stream it
        foreach ($activities as $activity) {
            $activityDom = $this->getXml($activity, $activity->transactions, $activity->results, $activity->organization->settings, $activity->organization);
            $activityXml = '';
            foreach ($activityDom->documentElement->childNodes as $node) {
                $activityXml .= $activityDom->saveXML($node) . "\n";
            }
            echo $activityXml;
            flush();
        }
