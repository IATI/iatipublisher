...
class ActivityRepository extends Repository
{
     ...
     */
    public function getActivityForOrganization(
        $organizationId,
        array $queryParams = [],
        int $page = 1
    ...
    public function importXmlActivities($activity_id, array $mappedActivity): Builder|Model|bool
    {
        ...
        $collaborationType = $this->autoFillSettingsValue(
            $this->getSingleValuedActivityElement($mappedActivity, 'collaboration_type'),
            'default_collaboration_type'
        );
        $defaultFlowType = $this->autoFillSettingsValue(
            $this->getSingleValuedActivityElement($mappedActivity, 'default_flow_type'),
            'default_flow_type'
        );
        $defaultFinanceType = $this->autoFillSettingsValue(
            $this->getSingleValuedActivityElement($mappedActivity, 'default_finance_type'),
            'default_finance_type'
        );
        $defaultTiedStatus = $this->autoFillSettingsValue(
            $this->getSingleValuedActivityElement($mappedActivity, 'default_tied_status'),
            'default_tied_status'
        );
        $defaultAidType = $this->autoFillDefaultAidTypeSettingValue(
            $this->getActivityElement($mappedActivity, 'default_aid_type')
        );
        ...
        $data = [
            ...
            'country_budget_items' => Arr::get(
                $this->getActivityElement($mappedActivity, 'country_budget_items'),
                '0',
                null
            ),
     ...
     * @throws JsonException
     */
    public function createOrUpdateActivities(int $organizationId, array $activitiesToUpsert): int
    {
        $preparedData = $this->prepareAllActivityDataToUpsert($organizationId, $activitiesToUpsert);

        if (empty($preparedData)) {
            return 0;
        }

        return $this->model->where('org_id', $organizationId)->upsert($preparedData, ['org_id', 'activity_identifier']);
    }

    /**
     * @throws JsonException
     */
    private function prepareAllActivityDataToUpsert(int $organizationId, array $activitiesToUpsert): array
    {
        $preparedData = [];

        foreach ($activitiesToUpsert as $activity) {
            if (is_array($activity)) {
                $activity['org_id'] = $organizationId;
                $activity['activity_identifier'] = Arr::get($activity, 'iati_identifier.activity_identifier');

                unset($activity['transactions']);
